...
.contButlerMgmt
  ...
  Modal.modalAddAccount(width="480", v-model="modalAddAccount",title="创建账户",:mask-closable="maskClosable",:footer-hide="true")
    comp-butler-add(ref="accountAdd",@refreshData="searchUserIdData",@closeModal="closeModal",v-if="modalAddAccount")

  <!-- 详情 抽屉 -->
  Drawer(:closable="true",width="640",v-model="drawerDetail",title="账户修改",@on-close="closeDrawerDetail",@on-visible-change="drawerChange",:mask-closable="maskClosable")
    comp-butler-modify(:userName="userName",:userMobile="userMobile",:userEmail="userEmail",:userCode="userCode",v-if="drawerDetail",ref="accountModify",@refreshData="searchUserIdData",)
</template>
